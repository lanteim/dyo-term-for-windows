...
window.I18N.register({
    en: { "widget.graph_connhist": "Connections History", "cat.monitoring": "Monitoring" },
    ru: { "widget.graph_connhist": "История соединений", "cat.monitoring": "Мониторинг" }
});
window.WIDGETS = window.WIDGETS || {};
window.WIDGETS.graph_connhist = {
    id: "graph_connhist",
    title: "widget.graph_connhist",
    category: "monitoring",
    description: "Rolling count of ESTABLISHED network connections",
    defaultSize: { w: 6, h: 3 },
    mount(body) {
        body.innerHTML = `
            <div style="position:relative;height:100%;display:flex;flex-direction:column">
              <div style="position:absolute;top:2px;left:4px;z-index:2;pointer-events:none">
                <span style="font-size:11px;color:var(--text-dim);letter-spacing:.05em">CONNS · ESTABLISHED</span>
        ...
        const tick = async () => {
            ...
            busy = true;
            try {
                const conns = await window.dyo.si("networkConnections");
                if (!alive) return;
                if (!Array.isArray(conns)) {
                    msgEl.textContent = "connection info unavailable";
                    return;
                }
                msgEl.textContent = "";
                const n = conns.filter(c => c && typeof c.state === "string" && c.state.toUpperCase() === "ESTABLISHED").length;
